...
Thực hiện qua AWS Console / CLI:
*   Tạo một DynamoDB Table với tên định nghĩa qua biến môi trường `MINIGAME_TABLE`.
*   Thiết lập Khóa chính (Partition Key: `PK`, Sort Key: `SK`) theo mô hình NoSQL đơn bảng (Single-Table Design) để tối ưu hóa truy vấn cho Sudoku, Minesweeper và Leaderboard.
...
*   Tạo danh sách các màn chơi thành các file `.json` sau đó đưa lên S3 theo đúng thư mục sẽ upload các file lên `MINIGAME_TABLE`.
![Dữ liệu màn chơi được đây lên bằng S3](https://res.cloudinary.com/lpwszzwp/image/upload/v1784651630/S3_item_ygzgfq.png)
**Bước 3: Cấu hình Backend Lambda Service**
*   Xây dựng các API xử lý logic game, cơ chế chống gian lận và tính toán điểm thưởng cho từng minigame (`minesweeperService.mjs`, `sudokuService.mjs`, `minigameService.mjs`).
![Các file xử lý logic game và ví dụ 1 hàm lamda](https://res.cloudinary.com/lpwszzwp/image/upload/v1784651630/S3_item_ygzgfq.png)
*   Cấu hình biến môi trường `.env` trên Lambda: `MINIGAME_TABLE`, `USER_TABLE`, và `GAME_SECRET_KEY`.
![Biến môi trường](https://res.cloudinary.com/lpwszzwp/image/upload/v1784651652/env_zrewsm.png)
**Bước 4: Thiết lập Worker định kỳ tự động cập nhật bảng xếp hạng (Leaderboard) mỗi 10 phút với AWS EventBridge**
*   Tạo một EventBridge Rule với lịch trình định kỳ: `rate(10 minutes)`.
*   Trỏ target của Rule về hàm Lambda chứa logic `handleLeaderboardWorker`.
![Hàm lamda tải bảng xếp hạng](https://res.cloudinary.com/lpwszzwp/image/upload/v1784651673/leaderboard_erwqsb.png)
**Bước 5: Khởi chạy Client Frontend (Electron & Redux)**
*   Khởi chạy ứng dụng Electron tích hợp Redux store (`minigameReducer.js`, `minigameLogsReducer.js`). 
*   Cấu hình các service gọi API (`minigameService.js`, `minesweeperService.js`, `sudokuService.js`) để kết nối với API Gateway.
...
### 4.2. Xây dựng Vòng lặp Gắn kết (Core Retention Loop) thông qua Kinh tế Ứng dụng
...
### 4.3. Rèn luyện Kỹ năng Mềm và Tư duy Logic cho Người học
