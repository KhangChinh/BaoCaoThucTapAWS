...
### 3. Mục tiêu dự án
Dự án hướng đến việc xây dựng một hệ sinh thái học tập cân bằng giữa **kỷ luật thép** và **giải trí lành mạnh**:
...
*   **Client (Frontend - Offline First):** Xây dựng bằng ReactJS và đóng gói bằng Electron thành ứng dụng Desktop độc lập. Quản lý trạng thái bằng Redux và Electron-store.
*   **AI & Tracking:** Sử dụng Browser Extension (AIGuard) giao tiếp qua WebSocket với Desktop App. Tích hợp Google Gemini (Cloud AI) hoặc Ollama (Local AI) cho các tính năng hỗ trợ học tập.
...
*   **Giai đoạn 2 (Phát triển Client):** Xây dựng giao diện ReactJS/Electron, tích hợp Focus Mode, hệ thống Daily Quest và cơ chế cache Offline-first.
...
*   **Tùy chọn AI linh hoạt:** Người dùng có thể tự cấu hình API Key (Gemini) hoặc chạy Local LLM (Ollama) miễn phí, giúp nền tảng không phải gánh chi phí LLM tập trung.
